...
int main() {
    ...
    double totales[NUM_PERSONAS] = {0}; //(POR ESO INICIALIZA EN 0)
     mostrarMenu();
    ...
    for(int i = 0; i < NUM_PERSONAS; i++){
        cout << "Comprador " << i + 1 << " " << nombres[i] << " total: $" << totales[i] << endl;
    }
    
    //Mostrar el total final vendido 
    double totalFinal = 0;
    for(int i = 0; i < NUM_PERSONAS; i++){
        totalFinal += totales[i];
    }
